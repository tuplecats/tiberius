...
async fn connect_asyncstd() -> Result<tiberius_asyncstd::Client> {
    LOGGER_SETUP.call_once(|| {
        env_logger::init();
    });
...
#[cfg(feature = "tls")]
async fn connect_with_full_encryption_smol() -> Result<()> {
    smol::run( async {
        LOGGER_SETUP.call_once(|| {
            env_logger::init();
        });
...
#[cfg(feature = "tls")]
#[async_std::test]
async fn connect_with_full_encryption_asyncstd() -> Result<()> {
    LOGGER_SETUP.call_once(|| {
        env_logger::init();
    });
